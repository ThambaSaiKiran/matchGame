...
class App extends Component {
  ...
  changeTime = () => {
    this.TimerId = setInterval(this.timerFun, 1000)
  }
  ...
  timerEndFunc = () => {
    ...
    return (
      <div className="gameEnd">
        ...
        <p>Your Score</p>
        <p>{score}</p>
        <button className="btn" type="button" onClick={this.playAgain}>
          <img
  ...
  render() {
    const {score, activeTab, randomImg, time} = this.state
    console.log(this.state)
    const activeImages = imagesList.filter(
      ...
      <div className="main">
        ...
        <div className="bg2">
          {time === 0 ? (
            ...
            <div>
              <img src={randomImg} alt="match" className="randomImg" />
              <div>
                <ul className="tabListCont">
                  {tabsList.map(eachItem => (
                    <TabItem
                      key={eachItem.tabId}
                      item={eachItem}
                      tabChangeFunc={this.changeActiveTab}
                    />
                  ))}
                </ul>
              </div>
              <div>
                <ul className="imgCont">
                  {activeImages.map(eachItem => (
                    <ImageItem
                      key={eachItem.id}
                      item={eachItem}
                      changeFunc={this.changeActiveImg}
                    />
                  ))}
                </ul>
              </div>
